...
[BUG 수정]
  transfer_staging_to_final()에 conn.commit() 추가
  psycopg2 기본값(autocommit=False): commit 없이 close() 호출 시 트랜잭션 자동 롤백
  cur.rowcount는 INSERT 시도 행 수를 반환하므로 로그엔 N rows 찍히지만 실제 DB엔 0건
"""
...
def transfer_staging_to_final(**context):
    ...
    conn.commit()   # FIX: commit 추가 — psycopg2 autocommit=False이므로 명시적 commit 필요
    cur.close()
    conn.close()
